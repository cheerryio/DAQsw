...
//{
//	u8 keyval = 0x00;
//	keyval = XGpio_DiscreteRead(&Key, 1);
//	return keyval;
//}
XUartLite uart0;
...
//		u_printf("TF Card Error !\r\n");
		...
		do
		{
			...
			vTaskDelay(500);
			tfCardSta = 0x02; // tf card bad
		}while(fr != FR_OK);

	}
	dcmLedSet &= 0x7f; // 0b0111_1111  dcm blue led turn on
	tfCardSta = 0x03; // tf card ok
...
static u8 tfCardPluginCheck()
{
	return (XGpio_DiscreteRead(&Key, 1) & 0x80);
}
static void tfCardCD()
{
	u8 tfsta =0;
	tfsta = tfCardPluginCheck() & 0x80;
	if(tfsta == 0x80)
	{
		do{
			tfsta = tfCardPluginCheck();
			vTaskDelay(500);
			tfCardSta = 0x01;// no TF Card
		}while(tfsta == 0x80);
	}
}
...
int main()
{
	...
	rtn = XGpio_Initialize(&Key, XPAR_PERIPHERALS_AXI_GPIO_KEYS_DEVICE_ID);
	configASSERT(rtn == XST_SUCCESS);
	XGpio_SetDataDirection(&Key, 1, 0xff);
