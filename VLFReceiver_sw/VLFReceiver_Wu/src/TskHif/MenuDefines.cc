...
// ======== menu 1 ========
const char *miFreqEditStr = (char *)"030000";
EzMenuItem<u8, MIValLen> miFreqEdit(miFreqEditStr, "000000", "256000", 0, 6);

const char *miDateTimeStr = (char *)"2020-01-01 00:00:00";
extern EzMenu<u8, MIValLen> menuDTEdit;
EzMenuItem<u8, MIValLen> miDateTime(miDateTimeStr, &menuDTEdit);

EzMenuItem<u8, MIValLen> *m1Items[M1ICount] = {&miFreqEdit, &miDateTime};
EzMenu<u8, MIValLen> menu1((char *)"", m1Items, M1ICount);

// ======== menu data time edit ========
const char *miYearEditStr = (char *)"00";
EzMenuItem<u8, MIValLen> miYearEdit(miYearEditStr, "00", "63", 0, 2);

const char *miMonthEditStr = (char *)"01";
EzMenuItem<u8, MIValLen> miMonthEdit(miMonthEditStr, "01", "12", 0, 2);

const char *miDayEditStr = (char *)"01";
EzMenuItem<u8, MIValLen> miDayEdit(miDayEditStr, "01", "31", 0, 2);

const char *miHourEditStr = (char *)"00";
EzMenuItem<u8, MIValLen> miHourEdit(miHourEditStr, "00", "23", 0, 2);

const char *miMinuteEditStr = (char *)"00";
EzMenuItem<u8, MIValLen> miMinuteEdit(miMinuteEditStr, "00", "59", 0, 2);

const char *miSecondEditStr = (char *)"00";
EzMenuItem<u8, MIValLen> miSecondEdit(miSecondEditStr, "00", "59", 0, 2);
...
EzMenu<u8, MIValLen> menuDTEdit((char *)"* Adj. Date & Time *", mDTEItems, MDTEICount);
...
void initMenu1()
{
	miFreqEdit.BColor = clrBlack;
	miFreqEdit.FColor = clrWhite;
	miFreqEdit.DisBColor = clrBlack;
	miFreqEdit.DisFColor = clrWhite;
	miFreqEdit.FocBColor = clrWhite;
	miFreqEdit.FocFColor = clrBlack;
	miFreqEdit.PosX = 7;
	miFreqEdit.PosY = 10;
	miFreqEdit.Width = 128;
	miFreqEdit.Height = 10;
	miFreqEdit.Prefix = (char *)"Carrier Freq:";
	miFreqEdit.CanFocus = true;
