...
int Ublox::encode(char c)
{
    ...
    else if(pos >= 120) //avoid a buffer overrun
    {
        memset(buf, '\0', 120);
        pos = 0;
        return 0;
...
int Ublox::process_buf()
{
    ...
    else if(strncmp(buf, "$GNGGA", 6) == 0)
    {
        read_gga();
        return RECV_MSG_GGA;	// GGA
    }
    else if(strncmp(buf, "$GNRMC", 6) == 0)
    
    {
        read_rmc();
        return RECV_MSG_RMC;	// RMC
    }
    else
    	return 0;
}
...
void Ublox::read_gga()
{
    ...
    while(tok.next(token, 20))
    {
        ...
        case 1: //time
        {
            ...
            float hms;
            hms = int(time);
            this->datetime.millis = time - hms;
            //this->datetime.seconds = fmod(hms, 100);
            hms=hms/100;
            //this->datetime.minutes = fmod(hms, 100);
            hms=hms/100;
            this->datetime.hours = hms;

            this->time_age = millis();
        }
        break;
        case 2: //latitude
        {
