...
class Eth{
	...
	public:
		 ...
		 * TcpClient. hpp
		 * 
		 * connect to the ip:port specified server
		 * @param server_ip_addr ip address of server
		 * @param port			 port number of server
		 * @return sock, this is the socket handler that would be used to send packet
		 */
		int connect_server(std::string server_ip_addr,u32 port);
		/**
		 * TcpClient. hpp
		 * 
		 ...
		 * @return bytes send
		 */
		int send_packet(int sock,void* buf,size_t send_size,u8_t apiflags=MSG_MORE);
	};
